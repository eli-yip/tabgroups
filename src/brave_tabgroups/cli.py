...
COLORS = {0: "grey", 1: "blue", 2: "red", 3: "yellow", 4: "green",
...
def default_sessions_dir(profile):
    ...
    candidates = [
        os.path.join(home, "Library/Application Support/BraveSoftware/Brave-Browser", profile, "Sessions"),       # macOS
        os.path.join(home, ".config/BraveSoftware/Brave-Browser", profile, "Sessions"),                            # Linux
        os.path.join(home, "AppData/Local/BraveSoftware/Brave-Browser/User Data", profile, "Sessions"),            # Windows
    ]
    for c in candidates:
        if os.path.isdir(c):
            return c
...
class Pickle:
    ...
    def str8(self):
        ...
        v = self.d[self.o:self.o + n].decode("utf-8", "replace"); self.o += n; self._align(); return v
    ...
    def str16(self):
        ...
        v = self.d[self.o:self.o + b].decode("utf-16-le", "replace"); self.o += b; self._align(); return v
...
def parse(data):
    ...
    for cid, c in cmds:
        try:
            ...
            if cid == CMD_SET_TAB_GROUP_METADATA2:
                hi = p.u64(); lo = p.u64(); title = p.str16(); color = p.u32()
                gmeta[(hi, lo)] = (title, color)
            elif cid == CMD_SET_TAB_GROUP:
                tid = p.i32(); p.i32()  # tid, reserved
                hi = p.u64(); lo = p.u64()
                tab_group[tid] = (hi, lo)
            elif cid == CMD_UPDATE_TAB_NAV:
                tid = p.i32(); idx = p.i32(); url = p.str8(); title = p.str16()
                navs[tid][idx] = (url, title)
            elif cid == CMD_SET_SELECTED_NAV_INDEX:
                tid = p.i32(); selected[tid] = p.i32()
            elif cid == CMD_SET_TAB_WINDOW:
                w = p.i32(); t = p.i32(); tab_window[t] = w
            elif cid == CMD_SET_TAB_INDEX:
                t = p.i32(); tab_index[t] = p.i32()
    ...
    result.sort(key=lambda g: min((t.get("window") or 0) for t in g["tabs"]) if g["tabs"] else 0)
    return {"version": version, "group_count": len(result),
            "tab_count": sum(len(g["tabs"]) for g in result), "groups": result}
...
def render_html(d):
    ...
    css_color = {"grey": "#9aa0a6", "blue": "#1a73e8", "red": "#d93025", "yellow": "#f9ab00",
                 "green": "#188038", "pink": "#d01884", "purple": "#9334e6", "cyan": "#007b83",
                 "orange": "#e8710a"}
    for i, g in enumerate(d["groups"], 1):
        name = html.escape(g["name"] or "(untitled)")
        col = css_color.get(g["color"], "#9aa0a6")
        parts.append(f"<h2><span class='chip' style='background:{col}'></span>{i}. {name} "
...
def main():
    ap = argparse.ArgumentParser(description="Export Brave tab groups from the session file.")
    ap.add_argument("--profile", default="Default", help="profile dir name (default: Default)")
    ap.add_argument("--session", help="explicit path to a Session_* file")
    ap.add_argument("--format", default="all", choices=["md", "json", "html", "csv", "all"])
    ap.add_argument("--out-dir", default="brave_tabgroups", help="output dir when --format all")
    args = ap.parse_args()
    ...
    if not session:
        ...
        if not sd:
            sys.exit(f"error: could not find Brave Sessions dir for profile {args.profile!r}")
    ...
    d = parse(data)
    sys.stderr.write(f"parsed {os.path.basename(session)}: "
    ...
    with open(os.path.join(args.out_dir, "tabgroups.csv"), "w", encoding="utf-8", newline="") as f:
        render_csv(d, f)
